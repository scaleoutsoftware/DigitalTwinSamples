 ...
 */
public class StatusTrackerMessageProcessor extends MessageProcessor<StatusTracker> implements Serializable {

    Gson gson = new Gson();

    @Override
    public ProcessingResult processMessage(ProcessingContext<StatusTracker> processingContext, StatusTracker digitalTwin, byte[] bytes) throws Exception {
        Logger logger = LogManager.getLogger(StatusTrackerMessageProcessor.class);
        try {
            StatusTrackerMessage msg = gson.fromJson(new String(bytes, StandardCharsets.UTF_8), StatusTrackerMessage.class);
            ...
            if(msg.offline() || msg.normalOperation()) {
                ...
                if(digitalTwin.experiencingMinorEvent() || digitalTwin.experiencingModerateEvent() ) {
            ...
            // the message indicates a moderate incident and this tracker has seen severe incidents
                ...
                digitalTwin.setAlertLevel(Constants.INFRASTRUCTURE_MODERATE_ALERTLEVEL, Constants.CONTROLLER_MODERATE_ALERTLEVEL+2);
            ...
            pw.flush();
            sw.flush();
            processingContext.logMessage(Level.SEVERE, sw.toString());
            logger.error(e);
            throw e;
        }
    }
