...
public class GasSensorMessage {
    ...
    public static byte[] serialize(GasSensorMessage message) {
        Gson gson = new Gson();
        return gson.toJson(message).getBytes(StandardCharsets.UTF_8);
    }

    public static GasSensorMessage deserialize(byte[] message) {
        Gson gson = new Gson();
        return gson.fromJson(new String(message, StandardCharsets.UTF_8), GasSensorMessage.class);
    }
}
