...
public class MyRealTimeTwinMessageProcessor extends MessageProcessor<MyRealTimeTwinModel, MyRealTimeTwinMessage> {
	...
	public ProcessingResult processMessages(ProcessingContext processingContext, MyRealTimeTwinModel twin, Iterable<MyRealTimeTwinMessage> messages) throws Exception {
		...
		for (MyRealTimeTwinMessage message : messages) {
			...
			processingContext.logMessage(Level.INFO, "The real-time digital twin '" + twin.Id + "' says '" + message.getStringPayload() + "'");
        }
